...
<script src="../js/admin-sidebar.js"></script>
 ...
 <button type="button" class="export-menu-item" onclick="exportReport('csv')">“„ &#128196; Export as CSV</button>
 <button type="button" class="export-menu-item" onclick="exportReport('xlsx')">“Š &#128202; Export as Excel</button>
 <button type="button" class="export-menu-item" onclick="exportReport('pdf')">“‘ &#128210; Export as PDF</button>
 </div>
 </div>
 </div>
 </form>
 ...
 <a href="<?= buildFilterUrl([], 'drill_' . $type) ?>" class="filter-chip-remove" title="Remove filter">&times;</a>
 </span>
 <?php endforeach; ?>
 ...
 <?php if ($hasActiveFilters): ?>
 &nbsp;&bull;&nbsp; <strong><?= count($activeFilters) ?> filter<?= count($activeFilters) > 1 ? 's' : '' ?> active</strong>
 <?php endif; ?>
 &nbsp;&bull;&nbsp; Comparing to: <strong><?= date('l, F j', strtotime($comparisonDates['start'])) ?> - <?= date('l, F j, Y', strtotime($comparisonDates['end'])) ?></strong>
 </div>
 ...
 <div class="stat-number"><?= formatMoney($totalCogs) ?></div>
 <div class="stat-change neutral"><?= $totalOrders > 0 ? round(($totalCogs / max($totalRevenue, 1)) * 100) : 0 ?>% of revenue</div>
 </div>
 <div class="stat-card <?= $totalProfit >= 0 ? 'success' : 'danger' ?>">
 <div class="stat-label">Gross Profit</div>
 <div class="stat-number"><?= formatMoney($totalProfit) ?></div>
 <div class="stat-change <?= $totalProfit >= 0 ? 'positive' : 'negative' ?>"><?= $totalRevenue > 0 ? round(($totalProfit / $totalRevenue) * 100) : 0 ?>% margin</div>
 </div>
 </div>
 ...
 <a href="../admin-orders.php?status=unpaid" class="outstanding-link">View in Orders &rarr;</a>
 </div>
 <?php endif; ?>
 ...
 <div class="empty-state"><div class="empty-state-icon">“Š</div><p>No event data available for this period</p></div>
 <?php else: ?>
 ...
 <div class="empty-state"><div class="empty-state-icon">“…</div><p>No data available for this period</p></div>
 <?php else: ?>
 <div class="table-container">
 <table class="data-table">
 ...
 <div class="empty-state"><div class="empty-state-icon">“</div><p>No size data available for this period</p></div>
 <?php else: ?>
 <div class="table-container">
 <table class="data-table">
 ...
 <a href="../admin-orders.php" class="page-btn">View All in Orders &rarr;</a>
 </div>
 <?php endif; ?>
 <?php endif; ?>
 </div>
