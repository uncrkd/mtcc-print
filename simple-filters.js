...
class SimpleFilterManager {
    constructor() {
        ...
        console.log('SimpleFilterManager with Pagination starting...');
        this.init();
    }
    ...
    init() {
        ...
        setTimeout(() => {
            if (window.analyticsManager && typeof window.analyticsManager.recalculateForEventsMode === 'function') {
                console.log('[Filters] Triggering initial analytics recalculation for active mode');
                window.analyticsManager.recalculateForEventsMode('active');
            }
        }, 2500);
        
        console.log('SimpleFilterManager with Pagination ready');
    }
    ...
    setupButtons() {
        ...
        document.querySelectorAll('[data-filter-type="priority"]').forEach(btn => {
            btn.addEventListener('click', () => {
                console.log('Priority button clicked:', btn.dataset.filterValue);
        ...
        document.querySelectorAll('[data-filter-type="status"]').forEach(btn => {
            btn.addEventListener('click', () => {
                console.log('Status button clicked:', btn.dataset.filterValue);
        ...
        document.querySelectorAll('[data-filter-type="duedate"]').forEach(btn => {
            btn.addEventListener('click', () => {
                console.log('Date button clicked:', btn.dataset.filterValue);
        ...
        document.querySelectorAll('[data-filter-type="prefix"]').forEach(btn => {
            btn.addEventListener('click', () => {
                console.log('Prefix button clicked:', btn.dataset.filterValue);
    ...
    setupEventsToggle() {
        ...
        if (!toggleContainer) {
            console.log('[Events Toggle] Container not found');
            return;
        }
        
        console.log('[Events Toggle] Setting up toggle');
        
        const buttons = toggleContainer.querySelectorAll('.segment-btn');
        buttons.forEach(btn => {
            btn.addEventListener('click', () => {
                const mode = btn.dataset.mode;
                console.log('[Events Toggle] Mode changed to:', mode);
    ...
    setupPagination() {
        ...
        if (perPageSelect) {
            perPageSelect.addEventListener('change', (e) => {
                ...
                console.log('Per page changed to:', this.pagination.perPage);
    ...
    toggleFilter(type, value, button) {
        ...
        console.log(`${type} filters now:`, Array.from(this.filters[type]));
        this.applyFilters();
    }
    ...
    applyFilters() {
        ...
        console.log('[applyFilters] Row event status counts:', { active: activeCount, archived: archivedCount, total: allRowsDebug.length });
        ...
        console.log(`Filtered to ${filteredRows.length} of ${allRows.length} rows`);
    ...
    setupSorting() {
        ...
        document.querySelectorAll('.date-sort-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                console.log('Date sort clicked:', column);
