...
function getSidebarNavItems() {
    ...
    // MTCC staff: add Revenue Reports link
    $isMtccStaffUser = (($_SESSION['admin_role'] ?? '') === 'mtcc_staff');
    if ($isMtccStaffUser && function_exists('hasPermission') && hasPermission('mtcc_analytics')) {
        $main['mtcc_reports'] = [
            'url' => 'admin/mtcc-reports.php',
            'label' => 'Revenue Reports',
            'icon' => '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="17" x2="8" y2="17"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="12" y1="11" x2="12" y2="17"/><line x1="9" y1="14" x2="15" y2="14"/></svg>',
        ];
    }
    if (!empty($main)) {
    ...
    // ---- MTCC View (god_mode / super_admin only — preview MTCC perspective) ----
    $currentRole = $_SESSION['admin_role'] ?? '';
    if (in_array($currentRole, ['god_mode', 'super_admin'])) {
        $groups['mtcc'] = [
            'label' => 'MTCC View',
            'items' => [
                'mtcc_dashboard' => [
                    'url' => 'admin-orders.php?view=mtcc',
                    'label' => 'MTCC Dashboard',
